...
def Main():
    print "Main"
    Initialize()
    loadMidi = "midi/WeWishYouAMerryChristmas.midi"
...
def play_note(note): # turns on note
    print "             play_note:",
    # add code here to turn on note/lights
    # all notes below 60 decimal will be lit as 60 (C4)
    # all notes above 72 decimal will be lit as 72 (C5)
    if note <= 60:
...
def stop_note(note): # turns off note
    ...
    # all notes below 60 decimal will be unlit as 60 (C4)
    # all notes above 72 decimal will be unlit as 72 (C5)
    if note <= 60:
